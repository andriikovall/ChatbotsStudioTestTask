...
const userDB = require('../db/entities/user');
...
module.exports.register = async function (req, res, next) {
  ...
  try {
    const excistingUser = await userDB.getByLogin(login);
    if (excistingUser) {
      res.status(409).json({
      ...
      await userDB.insert({ login, passwordHash });
      res.json({
...
module.exports.login = async function (req, res, next) {
  ...
  const passwordHash = hashFunc(password);
  try {
    const futureUser = await userDB.getByLoginAndPasswordHash(login, passwordHash);
    if (futureUser) {
